...
from ..utils.blender_helpers import append_history

# Generate Scene operator
class CANVAS3D_OT_GenerateScene(bpy.types.Operator):
...
class CANVAS3D_OT_LoadSpec(bpy.types.Operator, ImportHelper):
...
class CANVAS3D_OT_GenerateVariants(bpy.types.Operator):
        ...
        try:
            ...
            try:
                ...
                if coll is not None and hasattr(coll, "clear"):
                    coll.clear()
                    setattr(context.scene, "canvas3d_variants_index", 0)
            except Exception:
                pass
                        ...
                        try:
                            ...
                            if variants_local:
                                ...
                                if coll_local is not None and hasattr(coll_local, "clear"):
                                    ...
                                    setattr(scn, "canvas3d_variants_index", 0)
                                    setattr(scn, "canvas3d_selected_variant_index", 0)
...
class CANVAS3D_OT_SelectVariant(bpy.types.Operator):
...
class CANVAS3D_OT_ApplyEnhancements(bpy.types.Operator):
...
class CANVAS3D_OT_MoreIdeas(bpy.types.Operator):
...
class CANVAS3D_OT_RefreshVariantsList(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if coll is not None and hasattr(coll, "clear"):
                ...
                setattr(context.scene, "canvas3d_selected_variant_index", idx_active)
        except Exception:
            pass
...
class CANVAS3D_OT_ClearVariants(bpy.types.Operator):
        ...
        try:
            ...
            try:
                ...
                if coll is not None and hasattr(coll, "clear"):
                    coll.clear()
                    setattr(context.scene, "canvas3d_variants_index", 0)
                    setattr(context.scene, "canvas3d_selected_variant_index", 0)
            except Exception:
                pass
            return {'FINISHED'}
        except Exception as e:
...
import json
from ..utils.blender_helpers import read_history
from ..utils.exporters import export_collection_gltf, export_collection_fbx, export_collection_usd
...
def _apply_local_overrides(spec: dict, scene) -> dict:
    """
    ...
    try:
        ...
        if scale != 1.0 and isinstance(out.get("lighting"), list):
            new_lights = []
            for L in out.get("lighting", []) or []:
                if isinstance(L, dict):
                    try:
                        val = float(L.get("intensity", 0.0))
                        L["intensity"] = max(0.0, val * scale)
                    except Exception:
                        pass
                new_lights.append(L)
...
class CANVAS3D_OT_LoadOverridesFromSelection(bpy.types.Operator):
...
class CANVAS3D_OT_RegenerateLocal(bpy.types.Operator):
...
class CANVAS3D_OT_HistoryRefresh(bpy.types.Operator):
...
class CANVAS3D_OT_HistoryRevert(bpy.types.Operator):
...
class CANVAS3D_OT_ExportLastScene(bpy.types.Operator):
